...
以下图使用 PlantUML 语法，可在 PlantUML、开发环境插件或对应渲染器中直接生成图片。
...
class "需求" as Requirement {
  requirementId
  version
  status
}

class "验收项" as AcceptanceItem {
  acceptanceItemId
  description
  verificationMethod
  status
...
class "受控研发上下文" as ContextPackage {
  contextPackageId
  version
  status
}

class "智能体工作流" as AgentWorkflow {
  workflowId
  version
  name
}

class "任务执行实例" as TaskExecution {
  executionId
  status
  startedAt
  endedAt
...
class "文档内容单元" as DocumentContentUnit {
  contentUnitId
  contentType
  sourceType
  status
...
class "软件基线" as SoftwareBaseline {
  softwareBaselineId
  version
  commitRange
  artifactDigest
...
class "文档基线" as DocumentBaseline {
  documentBaselineId
  version
  approvedAt
}
