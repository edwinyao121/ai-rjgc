...
说明：**负责**表示直接发起或完成用例；**审批**表示作出关键结论；**协同**表示提供输入或处理反馈；**支撑**表示提供工程系统能力。

| 用例 | 项目负责人 | 开发人员 | 云端总控智能体 | 本地智能体 | 质量人员 | 外部系统 |
| --- | --- | --- | --- | --- | --- | --- |
| UC-01 建立并发布项目研发基线 | 审批 |  | 负责 |  | 协同 | 支撑 |
| UC-02 分析并确认需求 | 审批 | 协同 | 负责 |  | 协同 |  |
| UC-03 规划任务、依赖与风险 | 审批 | 协同 | 负责 |  | 协同 |  |
| UC-04 生成并下发任务执行包 | 协同 | 接收 | 负责 |  |  |  |
| UC-05 接收任务并准备执行环境 |  | 负责 |  | 协同 |  | 支撑 |
| UC-06 制订并确认实现计划 | 审批 | 负责 |  | 负责 |  |  |
| UC-07 执行编码、测试与自检 |  | 负责 |  | 负责 |  | 支撑 |
| UC-08 上报过程轨迹与成果 |  | 负责 | 接收 | 负责 |  |  |
| UC-09 执行云端智能审查 | 协同 |  | 负责 |  | 协同 |  |
| UC-10 执行质量审查与门禁判定 | 审批 |  | 协同 |  | 负责 | 支撑 |
| UC-11 生成整改任务并闭环 |  | 负责 | 负责 | 协同 | 审批 |  |
| UC-12 提交变更并触发流水线 |  | 负责 |  | 协同 |  | 支撑 |
| UC-13 合并变更并形成制品 | 审批 | 协同 |  |  | 审批 | 负责 |
| UC-14 推进阶段并归档 | 负责 |  | 协同 |  | 协同 | 支撑 |
| UC-15 提炼知识候选 | 协同 | 协同 | 负责 |  | 协同 |  |
| UC-16 审核并发布项目知识 | 审批 |  | 协同 |  | 审批 |  |
