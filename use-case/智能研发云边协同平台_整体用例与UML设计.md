...
| 主要参与者 | 项目负责人、云端总控智能体 |
| 触发条件 | 新项目启动，或项目规范、架构边界、质量要求需要建立或更新。 |
...
| 触发条件 | 提交新需求，或需求范围、验收标准、工程等级、交付方式发生变化。 |
...
| 触发条件 | 本地客户端收到已批准任务包，且开发人员确认开始执行。 |
...
| 主要参与者 | 开发人员、本地智能体、外部系统 |
| 触发条件 | 任务执行实例启动，或本地执行产生新的过程记录和研发产物。 |
...
| 触发条件 | 预审、测试、部署或一致性校核发现问题，或质量人员退回整改。 |
...
| 触发条件 | 质量门禁通过，且软件、文档、制品和交付验证达到基线候选状态。 |
...
场景特色用例不另起一条独立生命周期，而是挂接到第 4 节的核心用例。每个特色用例保留统一七字段，具体场景级细节仍以四份来源文档为准。
...
| 主要参与者 | 项目负责人、云端总控智能体、质量人员 |
| 触发条件 | 新项目建立，或项目规范、工作流、人工卡点需要新增或变更。 |
...
| 触发条件 | 需求确认并进入任务规划，或出现共享资产和并行执行风险。 |
...
| 触发条件 | 整改闭环完成、识别出重复问题，或形成可复用工程经验。 |
...
| 主要参与者 | 开发人员、本地智能体、外部系统 |
| 触发条件 | 生成策略、任务包和本地上下文准备完成，且获得生成授权。 |
...
| 触发条件 | 测试任务或测试计划确认，且测试环境和被测版本准备就绪。 |
...
| 主要参与者 | 开发人员、本地智能体、质量人员、外部系统 |
| 触发条件 | 技术归因和修复授权完成，且原失败用例和影响范围已明确。 |
...
| 触发条件 | 新任务建立，或需求、代码、测试、文档、软件版本发生变化。 |
...
| 触发条件 | 软件和文档候选版本准备完成，进入发布前或基线前一致性校核。 |
