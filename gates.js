  ...
  function renderAuditCard(task) {
    ...
    const stageSections = stageBlocks.map(block => {
      ...
      const gateItems = block.defs.map((g, j) => {
        ...
        if (task.pid === 'p5') {
          const p5GateResults = {
            '需求完整性检查': '包含大模型文档摘要、既定流转顺序、双核验下载及印章审计的电子公文管理系统需求规范检查已通过',
            '语义冲突检测': (function() {
            ...
            '可追溯性检查': '已完成“双因子(2FA)下载”、“流转状态机”及“多级印章审批”到国家电子公文安全标准的双向追溯审计',
            ...
            '技术选型评估': '已验证国产OFD阅读器兼容性及基于国密国标SM2/SM3算法的电子签名与证书安全集成可行性',
            '编码规范检查': '国密签名组件及流转切面规范检查 0 errors, 0 warnings，规范评级 A',
            '安全漏洞扫描': 'OWASP 安全扫描 100% 通过，绝密级公文防泄密沙箱及越权检查无任何中高危漏洞',
            ...
            '合规审计检查': '国家机密级电子公文流转审计追溯完成，已生成安全电子印章日志与唯一档案归档编号 SEAL-2026-0038491'
          };
          ...
          const p5GateEvidenceFiles = {
            ...
            '技术选型评估': '国密算法及OFD选型对比分析.xlsx',
            '编码规范检查': '公文系统代码规范扫描报告.html',
            '安全漏洞扫描': '公文系统防泄露漏洞扫描报告.pdf',
