...
智能编码阶段的测试边界以单元测试、组件测试、纯函数/数据处理测试和轻量集成测试为主，`manifest.test` 应能运行这些快速验证。Playwright 端到端测试归属测试质检阶段，不作为智能编码阶段的强制新增产物；若编码阶段发现项目已有端到端测试，不得删除或弱化。

智能编码阶段 opencode 命令超时时间为 2 小时。其他 opencode 研发阶段如无单独说明，沿用默认阶段超时；需求澄清阶段继续使用 15 分钟超时策略。
