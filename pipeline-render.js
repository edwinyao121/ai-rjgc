...
function renderTabContent(task, tab) {
  ...
  if (tab === 'gates') {
    ...
    const activityHtml = stageLogs.length === 0
      ...
      : `<div class="activity-feed" id="activityFeed" style="max-height:200px;">${stageLogs.map((log, index) => `
          <div class="activity-item ${log.type} fade-in-pending" data-index="${index}" style="opacity:0;">
            <span class="activity-time">${log.time}</span>
            <span class="activity-icon ${log.iconType}">${log.icon}</span>
            <span class="activity-text">${log.text}</span>
          </div>`).join('')}</div>`;
    ...
    const feedContainer = document.getElementById('activityFeed');
    if (feedContainer) {
      const items = feedContainer.querySelectorAll('.fade-in-pending');
      items.forEach((item, index) => {
        setTimeout(() => {
          item.style.opacity = '1';
          item.classList.add('fade-in-active');
          item.classList.remove('fade-in-pending');
        }, 300 * (index + 1));
      });
    }
...
function getArtifacts(stageName) {
  ...
  if (task && task.pid === 'p5') {
    const map = {
      ...
      '单元测试': [
        { icon: '&#9881;', name: '单元测试报告.html', meta: '32KB · 5项安全核验用例全部通过' },
...
<span class="keyword">public class</span> <span style="color:#F9A8D4;">QueryOptimizer</span> {
  ...
  } else if (name === 'UI测试报告.html') {
    preview.innerHTML = `
      <div style="margin-top:12px;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;background:#ECFDF5;border:1px solid #A7F3D0;border-radius:8px;margin-bottom:12px;">
          <div>
            <div style="font-weight:700;font-size:14px;color:#065F46;">&#9745; UI测试报告.html</div>
        ...
        <div class="grid-70-30" style="grid-template-columns: 280px 1fr; gap:16px; min-height: 520px; background:var(--surface); border: 1px solid var(--border); border-radius:12px; overflow:hidden;">
          ...
          <div style="background:#F8FAFC; border-right: 1px solid var(--border); padding: 16px; display:flex; flex-direction:column; gap:8px;">
            ...
            <div class="case-item active" id="case-btn-immutable" onclick="window.switchTestEvidence('immutable')" style="display:flex; flex-direction:column; gap:4px; padding:10px 12px; border-radius:8px; border:1px solid var(--primary); background:#EFF6FF; cursor:pointer; transition:var(--transition); user-select:none;">
              <div style="display:flex; align-items:center; justify-content:space-between;">
                <span style="font-size:12px; font-weight:700; color:var(--primary);">testRegistrationImmutable</span>
            ...
            <div class="case-item" id="case-btn-2fa" onclick="window.switchTestEvidence('2fa')" style="display:flex; flex-direction:column; gap:4px; padding:10px 12px; border-radius:8px; border:1px solid var(--border); background:var(--surface); cursor:pointer; transition:var(--transition); user-select:none;">
              <div style="display:flex; align-items:center; justify-content:space-between;">
                <span style="font-size:12px; font-weight:700; color:var(--text);">testDocDownload2FA</span>
                <span style="color:var(--success); font-size:11px; font-weight:700;">● 通过</span>
              </div>
    ...
    setTimeout(function() {
      if (typeof window.switchTestEvidence === 'function') {
        window.switchTestEvidence('immutable');
      }
    }, 50);
...
window.showEvidenceLightbox = function(caseId) {
  ...
  if (caseId === '2fa') {
    ...
    mockupHtml = `<img src="docs/screenshot/截图 2026-05-20 16-33-09.png" style="max-width:100%; border-radius:8px; display:block;" alt="二次身份核验截图">`;
  } else if (caseId === 'seq') {
    caseTitle = '阅件流转顺序强控 - 证据截图';
    mockupHtml = `<img src="docs/screenshot/截图 2026-05-20 16-33-16.png" style="max-width:100%; border-radius:8px; display:block;" alt="阅件流转顺序强控截图">`;
  } else if (caseId === 'immutable') {
    caseTitle = '登记基本信息不可篡改 - 证据截图';
    mockupHtml = `<img src="docs/screenshot/截图 2026-05-20 16-33-22.png" style="max-width:100%; border-radius:8px; display:block;" alt="登记基本信息不可篡改截图">`;
  } else if (caseId === 'stamp') {
    caseTitle = '电子公章印章用印与归档 - 证据截图';
    mockupHtml = `<img src="docs/screenshot/截图 2026-05-20 16-33-34.png" style="max-width:100%; border-radius:8px; display:block;" alt="用印审批截图">`;
  } else if (caseId === 'version') {
    caseTitle = '档案版本历史树状回溯 - 证据截图';
mockupHtml = `<img src="docs/screenshot/截图 2026-05-20 16-33-40.png" style="max-width:100%; border-radius:8px; display:block;" alt="档案版本控制截图">`;
  }
